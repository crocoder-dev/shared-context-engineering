...
`invocation input` is optional. Split it into two parts before invoking the skill:
...
Empty `invocation input` is valid. It selects the regular path with no commit
...
For example: `$sce-commit oneshot`.
