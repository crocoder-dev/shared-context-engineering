...
`invocation input` is `[rebuild] [path ...]`. Parse it into two parts before any
investigation:
...
- Empty `invocation input` is valid and selects additive mode with no extra paths.
...
For example: `$sce-brownfield rebuild docs/`.
