...
field is missing or not `synced`. Before returning `validated`, write the
plan-level context synchronization field as `pending` in the plan file; this
must happen before plan context synchronization is invoked. If either durable
check or write cannot be completed, return `blocked`.
...
remains incomplete, or when any completed task's `Context synchronization` field
is missing or not `synced`.
...
### 6. Determine context impact for the handoff
