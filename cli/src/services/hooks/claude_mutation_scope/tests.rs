...
fn post_tool_use_and_failure_and_permission_denied_share_tool_identity_shape() {
    for event_name in [
        ...
        let payload =
            pre_tool_use_json(&[(HOOK_EVENT_NAME_FIELD, Value::String(event_name.to_string()))]);
...
mod driver {
    ...
    fn subagent_without_exact_model_state_does_not_inherit_main_model() {
        ...
        for (tool_use_id, agent_id) in [("toolu_main", None), ("toolu_subagent", Some("agent-1"))] {
    ...
    fn missing_or_failed_model_resolution_keeps_session_provenance_and_allows_start() {
        ...
        for tool_use_id in ["toolu_missing", "toolu_failed"] {
            let payload =
                pre_tool_use_json(&[(TOOL_USE_ID_FIELD, Value::String(tool_use_id.to_string()))]);
    ...
    fn read_only_tool_creates_no_scope_and_never_touches_the_seam_or_git_dir() {
        ...
        let payload = pre_tool_use_json(&[(TOOL_NAME_FIELD, Value::String("Read".to_string()))]);
    ...
    fn delegation_tool_creates_no_scope_ac3() {
        ...
        let payload = pre_tool_use_json(&[(TOOL_NAME_FIELD, Value::String("Agent".to_string()))]);
    ...
    fn linked_worktree_cwd_and_git_dir_are_never_conflated_for_pre_tool_use_start() {
        ...
        let state = state::read_state(&git_dir).expect("state should be readable under git_dir");
        assert_eq!(
            state.attempts.len(),
            1,
    ...
    fn post_tool_use_with_no_live_attempt_is_a_safe_no_op_d9() {
        ...
        let resolver = fixed_resolver(std::env::temp_dir().join("sce-unused-nonexistent-git-dir"));
...
mod production_regressions {
    ...
    fn background_pre_tool_use_for(
        ...
        let mut object: serde_json::Map<String, Value> = serde_json::from_str(&pre_tool_use_for(
            cwd,
            session_id,
            "Bash",
            tool_use_id,
    ...
    fn test12_pending_start_crash_before_start_is_recovered_conservatively() {
        ...
        let allocated =
            state::allocate_attempt(&git_dir, &key, "Write").expect("allocation should succeed");
