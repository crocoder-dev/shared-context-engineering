use super::events::{
    classify_tool, format_opencode_scope_id, opencode_scope_close_event_id,
    opencode_scope_provenance, opencode_scope_start_event_id, parse_opencode_hook_event,
    AttemptKey, OpenCodeHookEvent, OpenCodeToolExecution, OpenCodeToolIdentity, ToolClassification,
    CALL_ID_FIELD, CWD_FIELD, HOOK_EVENT_NAME_FIELD, HOOK_EVENT_SERVER_DISPOSED,
    HOOK_EVENT_SESSION_DELETED, HOOK_EVENT_SESSION_ERROR, HOOK_EVENT_SESSION_IDLE,
    HOOK_EVENT_SHELL_ENV, HOOK_EVENT_TOOL_ERROR, HOOK_EVENT_TOOL_EXECUTE_AFTER,
    HOOK_EVENT_TOOL_EXECUTE_BEFORE, MODEL_FIELD, SESSION_ID_FIELD, TOOL_NAME_FIELD,
};
use super::lifecycle::{run_opencode_mutation_scope_from_payload, FAIL_CLOSED_MESSAGE};
...
fn missing_required_fields_are_rejected_without_fabricating_identity() {
    for field in [SESSION_ID_FIELD, CALL_ID_FIELD, CWD_FIELD, TOOL_NAME_FIELD] {
        let mut object: Map<String, Value> =
            serde_json::from_str(&tool_event_json(HOOK_EVENT_TOOL_EXECUTE_BEFORE, &[])).unwrap();
        object.remove(field);
...
fn shell_env_parses_without_a_tool_name() {
    ...
    let OpenCodeHookEvent::ShellEnv(shell) = parse_opencode_hook_event(&payload).unwrap() else {
...
fn tool_execute_after_parses_tool_identity() {
    let OpenCodeHookEvent::ToolExecuteAfter(identity) =
        parse_opencode_hook_event(&tool_event_json(HOOK_EVENT_TOOL_EXECUTE_AFTER, &[])).unwrap()
    else {
...
mod lifecycle_tests {
    ...
    use super::super::state::{read_state, AttemptPhase, RecoveryState};
...
mod runtime_seam_tests {
    ...
    use crate::services::hooks::opencode_mutation_scope::lifecycle::{
    ...
    fn regression_f_untracked_tool_error_is_zero_footprint() {
        ...
        assert!(!adapter_state_dir(&repo.git_dir())
    ...
    fn regression_d_concurrent_survivor_stays_usable_after_a_transient_cleanup_failure() {
        ...
        assert_eq!(
            ...
            Some(AttemptPhase::PendingAbandon),
