...
const PROBE01_APPLY_PATCH_PRE: &str =
    include_str!("fixtures/probe01-apply-patch-and-shell-success.apply_patch.pre_tool_use.json");
const PROBE01_STOP: &str = include_str!("fixtures/probe01-apply-patch-and-shell-success.stop.json");
...
const PROBE04_BLOCKED_PRE: &str =
    include_str!("fixtures/probe04-pre-tool-use-hook-hookspecificoutput-deny.pre_tool_use.json");
...
const PROBE12_MCP_PRE: &str = include_str!("fixtures/probe12-mcp-mutate-success.pre_tool_use.json");
...
fn ac2_session_end_needs_no_turn_id() {
    let CodexHookEvent::SessionEnd(identity) = parse_codex_hook_event(PROBE01_SESSION_END).unwrap()
    else {
...
mod driver {
    ...
    fn recovery_barrier_stays_closed_when_flush_fails_ac12() {
        ...
        std::fs::create_dir_all(&git_dir).expect("git dir should be created");
        let resolver = fixed_resolver(git_dir.clone());
        let generation = state::arm_recovery(&git_dir).expect("arming the barrier should succeed");
    ...
    fn test_h_cleanup_owning_the_boundary_lock_blocks_admission_until_recovery_is_processed() {
        ...
        let abandon_at = first_index_of(&recorded, "abandon").expect("cleanup abandoned exec-a");
    ...
    fn seed_orphaned_flushing(git_dir: &Path) -> u64 {
        ...
        match state::admit_tracked_attempt(git_dir, &key("seed", None, "seed"), "seed-turn", "Bash")
    ...
    fn test_i_orphaned_flushing_is_reclaimed_and_flush_is_retried_once() {
        ...
        assert!(first_index_of(&ops, "flush").unwrap() < first_index_of(&ops, "start").unwrap());
    ...
    fn untracked_post_tool_use_never_touches_mutation_scope_machinery() {
        ...
        for tool in [
            ...
            let payload = post_tool_use_json(&[(TOOL_NAME_FIELD, Value::String(tool.to_string()))]);
...
mod production_regressions {
    ...
    const PROBE02_FAILED_SHELL_PRE: &str =
        include_str!("fixtures/probe02-shell-partial-write-then-nonzero-exit.pre_tool_use.json");
    const PROBE06_APPLY_PATCH_FAILURE_PRE: &str =
        include_str!("fixtures/probe06-apply-patch-verification-failure-no-post.pre_tool_use.json");
    ...
    const PROBE13_MCP_STOP: &str = include_str!("fixtures/probe13-mcp-mutate-then-error.stop.json");
    ...
    impl CodexRepo {
        ...
        fn adapter_state_at(root: &Path) -> state::AdapterState {
            state::read_state(&Self::git_dir_at(root)).expect("adapter state should be readable")
        }
    ...
    fn bash_call<'a>(cwd: &'a str, session_id: &'a str, tool_use_id: &'a str) -> TrackedCall<'a> {
        TrackedCall {
            cwd,
            session_id,
            turn_id: "turn-1",
