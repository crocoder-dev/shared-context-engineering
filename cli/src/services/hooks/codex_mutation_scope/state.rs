...
pub(crate) fn allocate_attempt(
    ...
    let scope_id = format_codex_scope_id(attempt_seq, key);
...
mod tests {
    ...
    fn attempt_seq_allocation_is_sequential_across_distinct_keys() {
        ...
        let first = allocate_attempt(&git_dir, &key("session-1", None, "exec-1"), "Bash")
            .expect("first allocation should succeed");
        let second = allocate_attempt(&git_dir, &key("session-1", None, "exec-2"), "apply_patch")
    ...
    #[test]
    fn attempt_state_is_checkout_local_to_its_git_dir() {
        ...
        allocate_attempt(&git_dir_a, &key("session-1", None, "exec-1"), "Bash")
            .expect("checkout A allocation should succeed");
        allocate_attempt(&git_dir_a, &key("session-1", None, "exec-2"), "Bash")
            .expect("second checkout A allocation should succeed");

        let first_b = allocate_attempt(&git_dir_b, &key("session-1", None, "exec-1"), "Bash")
            .expect("checkout B allocation should succeed");
        assert_eq!(
            first_b.attempt.attempt_seq, 1,
            "checkout B's monotonic counter must be independent of checkout A's"
        );

        let state_a = read_state(&git_dir_a).expect("state A readable");
        let state_b = read_state(&git_dir_b).expect("state B readable");
        assert_eq!(state_a.attempts.len(), 2);
        assert_eq!(state_b.attempts.len(), 1);
    ...
    fn duplicate_live_attempt_reuses_the_same_attempt_seq_and_scope_id() {
        ...
        let first = allocate_attempt(&git_dir, &attempt_key, "Bash")
            .expect("first allocation should succeed");
        let second = allocate_attempt(&git_dir, &attempt_key, "Bash")
        ...
        assert_eq!(
            ...
            "duplicate delivery must reuse the same attempt_seq"
        );
        assert_eq!(
            first.attempt.scope_id, second.attempt.scope_id,
            "duplicate delivery must reuse the same ScopeId"
        );
    ...
    fn mark_active_transitions_phase_from_pending_start_to_active() {
        ...
        std::fs::create_dir_all(&git_dir).expect("git dir should be created");
        let allocated = allocate_attempt(&git_dir, &key("session-1", None, "exec-1"), "Bash")
    ...
    fn mark_active_for_an_unknown_scope_id_is_rejected_without_fabricating_an_attempt() {
        ...
        let state = read_state(&git_dir).expect("state should be readable");
        assert!(state.attempts.is_empty(), "no attempt may be fabricated");
    ...
    fn state_round_trips_durably_through_the_canonical_path() {
        ...
        let allocated = allocate_attempt(
            &git_dir,
    ...
    fn a_leftover_lock_file_with_no_active_os_lock_does_not_block_a_new_acquirer() {
        ...
        let result = allocate_attempt(&git_dir, &key("session-1", None, "exec-1"), "Bash");
        assert!(
            result.is_ok(),
        ...
        let handles: Vec<_> = (0..PARALLEL_WRITER_COUNT)
            .map(|index| {
                ...
                thread::spawn(move || {
                    ...
                    allocate_attempt(&git_dir, &attempt_key, "Bash")
    ...
    #[test]
    fn every_public_helper_releases_the_state_lock_before_returning() {
        let git_dir = unique_test_git_dir("lock-released-between-helpers");
        std::fs::create_dir_all(&git_dir).expect("git dir should be created");
        let attempt_key = key("session-1", None, "exec-1");

        let allocated = allocate_attempt(&git_dir, &attempt_key, "Bash")
            .expect("allocate_attempt must release the lock before returning");
        mark_active(&git_dir, &allocated.attempt.scope_id)
            .expect("mark_active must acquire the lock allocate_attempt released");
        mark_recovery_pending(&git_dir)
            .expect("mark_recovery_pending must acquire the lock mark_active released");
        clear_recovery_pending(&git_dir)
            .expect("clear_recovery_pending must acquire the lock mark_recovery_pending released");
        remove_attempt(&git_dir, &allocated.attempt.scope_id)
            .expect("remove_attempt must acquire the lock clear_recovery_pending released");
    ...
    fn mark_recovery_pending_arms_the_barrier_without_touching_attempts() {
        ...
        let state = read_state(&git_dir).expect("state should be readable");
        assert!(state.recovery_pending, "D13: the barrier must be armed");
        assert_eq!(
            state.attempts.len(),
            1,
    ...
    fn adapter_state_files_live_only_below_git_dir_sce() {
        ...
        allocate_attempt(&git_dir, &key("session-1", None, "exec-1"), "Bash")
            .expect("allocation should succeed");
