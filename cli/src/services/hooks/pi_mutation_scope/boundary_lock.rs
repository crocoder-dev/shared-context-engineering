...
const ADAPTER_BOUNDARY_LOCK_FILE: &str = "pi-mutation-scope-boundary.lock";
...
mod tests {
    ...
    fn unique_git_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "sce-pi-mutation-scope-boundary-{label}-{}-{id}",
            std::process::id()
        ))
    }
    ...
    fn the_boundary_lock_path_is_distinct_from_the_state_lock_and_lives_under_sce() {
        ...
        assert_ne!(
            path.file_name(),
            Path::new("pi-mutation-scope-state.lock").file_name(),
        );
    }
}
