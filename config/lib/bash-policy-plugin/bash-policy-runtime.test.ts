...
describe("parseCommandSegments", () => {
  ...
  describe("double operators (&&, ||)", () => {
    test("splits on &&", () => {
      ...
      expect(result).toEqual([["git", "status"], ["npm", "install"]]);
    });

    test("splits on ||", () => {
      const result = parseCommandSegments("git status || echo 'failed'");
      expect(result).toEqual([["git", "status"], ["echo", "failed"]]);
    });

    test("handles mixed && and ||", () => {
      const result = parseCommandSegments("git add . && git commit -m 'test' || echo 'failed'");
      expect(result).toEqual([["git", "add", "."], ["git", "commit", "-m", "test"], ["echo", "failed"]]);
    });
  });
  ...
  describe("background operator", () => {
    ...
    test("handles command followed by background", () => {
      ...
      expect(result).toEqual([["npm", "start"], ["echo", "done"]]);
    });
  });

  describe("complex commands", () => {
    test("handles multiple operator types", () => {
      const result = parseCommandSegments("cat abc | git diff && npm run build; ls &");
      expect(result).toEqual([["cat", "abc"], ["git", "diff"], ["npm", "run", "build"], ["ls"]]);
    });
  ...
  describe("with quotes", () => {
    test("preserves quoted arguments", () => {
      ...
      expect(result).toEqual([["echo", "hello world"], ["wc", "-l"]]);
    });
