...
- For OTLP header authentication, keep `OTEL_EXPORTER_OTLP_HEADERS` env-only and secret-bearing: expose only redacted presence/provenance in `sce config show`, never add header values to `.sce/config.json` or generated schemas, parse standard comma-separated `key=value` pairs only when OTEL is enabled, reject malformed/empty-key/duplicate-key input without echoing raw header material, and apply valid headers to both `grpc` and `http/protobuf` exporters.
