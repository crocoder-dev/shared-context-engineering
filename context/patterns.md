...
- For the current local-hook baseline, keep `pre-commit`, `post-commit`, and `post-rewrite` as deterministic no-op entrypoints; keep `diff-trace` as an explicit STDIN intake path with deterministic required-field validation and `context/tmp/[timestamp]-diff-trace.json` persistence.
