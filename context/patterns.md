...
- Route local Agent Trace SQLite persistence through a dedicated service module (`cli/src/services/local_db.rs`) so command handlers do not expose low-level SQL/connection details.
...
- For the current local-hook baseline, keep `pre-commit`, `post-commit`, and `post-rewrite` as deterministic no-op entrypoints until an explicitly approved tracing redesign lands.
...
- For the current local DB baseline, use repo-local SQLite at `.sce/local.db`, create parent directories before first use, bootstrap schema idempotently in `cli/src/services/local_db.rs`, keep SQL/schema ownership localized to that module, expose typed CRUD/query helpers for milestone Agent Trace entities, and provide submit-time orchestration helpers that lazily ensure DB/session/conversation before prompt persistence.
- For prompt-submit integration in the current OpenCode runtime, treat plugin `chat.message` as the submit seam and keep TypeScript bridge logic thin + fail-open: extract prompt text parts, call `sce trace append-prompt --prompt <text>` via child process from the repo worktree, and keep all persistence orchestration in Rust (`cli/src/services/trace.rs` + `cli/src/services/local_db.rs`).
