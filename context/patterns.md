...
- For hook setup CLI UX, treat `--hooks` as a dedicated setup mode with optional `--repo <path>`, enforce deterministic option compatibility (`--repo` requires `--hooks`; no `--hooks` + target-flag mixes), and emit stable per-hook status + backup lines for automation-friendly logs.
