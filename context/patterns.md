...
- For required-hook setup execution, resolve repository root and effective hooks directory from git (`rev-parse --show-toplevel`, `rev-parse --git-path hooks`), then apply deterministic per-hook outcomes (`Installed`, `Updated`, `Skipped`) with staged writes, executable-bit enforcement, and remove-and-replace behavior that removes existing hooks before swapping staged content. For internal previews, reuse the same hook path/content/executable-state interpretation but return planned `Installed`/`Updated`/`Skipped` outcomes without creating directories, staging files, or replacing hooks.
...
- For lifecycle-owned hook diagnosis, keep repository targeting and report assembly in doctor, but keep required-hook presence/executable/content/read checks in `cli/src/services/hooks_lifecycle.rs`; map lifecycle diagnostics back into existing doctor problem/report types without changing doctor text/JSON output contracts.
- For lifecycle-owned hook repair, keep doctor responsible for selecting fixable `hook_rollout` problems and rendering existing `DoctorFixResultRecord` values, but obtain hook repair through `LifecycleRegistry::fix_lifecycle(HOOKS_SERVICE_ID)` and map lifecycle action outcomes back into the existing fixed/skipped/failed vocabulary and detail strings.
...
- For internal service lifecycle work, use `cli/src/services/lifecycle.rs` as the common trait/model seam: keep setup/diagnose/fix/preview facets composable, pass runtime dependency paths through typed request contexts, return typed reports/action plans, and map those reports to command-specific setup/doctor render types only at orchestration boundaries. Hooks are the implemented pilot in `cli/src/services/hooks_lifecycle.rs`; local DB is the second implemented service in `cli/src/services/local_db_lifecycle.rs` with diagnostic and fix lifecycle facets registered in the registry (setup and preview deferred because setup already owns `bootstrap_local_db`); future services should follow the same pattern of delegating mutations to canonical service-owned helpers while exposing non-mutating preview/action-plan data separately. Registry lookup should stay narrow and explicit: `cli/src/services/lifecycle_registry.rs` registers hooks and local DB, and additional service IDs should be added only when their lifecycle implementation task is approved. When adding a new lifecycle service: (1) define a `ServiceId` constant and implement the needed lifecycle traits in a dedicated module under `cli/src/services/`, (2) register the new service ID and its implemented facets in `LifecycleRegistry`, (3) wire command orchestration to resolve the capability through the registry and map typed lifecycle reports into existing command-specific output types, (4) keep mutations delegated to canonical service-owned helpers while exposing non-mutating preview/action-plan data separately, and (5) add registry lookup tests for the new service ID and missing-capability handling.
- For lifecycle-owned local DB diagnosis, keep path resolution and parent-directory readiness checks in `cli/src/services/local_db_lifecycle.rs`; map lifecycle diagnostics back into existing doctor problem/report types without changing doctor text/JSON output contracts. The local DB diagnostic lifecycle uses `default_paths::local_db_path()` for canonical path resolution and produces stable diagnostic kind constants (`local_db_path_unresolvable`, `local_db_parent_missing`, `local_db_parent_not_directory`, `local_db_parent_not_writable`, `local_db_health_check_failed`).
