...
- For `diff-trace` hook intake, keep producer-facing failure behavior fail-open: STDIN read, parse/validation, attribution lookup, artifact persistence, and setup/persistence failures are logged with `sce.hooks.diff_trace.error` and converted into command success; preserve the existing valid-payload success text and the post-artifact AgentTraceDb write-warning success path.
