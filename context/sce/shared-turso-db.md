...
- `EncryptedTursoDb<M>` also exposes retry-backed synchronous `execute()`, `query()`, and `query_map()` wrappers plus generic `run_migrations()` with the same `__sce_migrations` metadata flow used by `TursoDb<M>`. Its operation methods use the same config-driven operation retry policy (resolved from `policies.database_retry.<db>.query` with hardcoded fallback defaults) as `TursoDb<M>`.
