...
- `cli/build.rs` scans immediate `cli/migrations/<db-name>/*.sql` directories at compile time and writes `cli/src/generated_migrations.rs` with database-named migration constants (`AGENT_TRACE_REPOSITORY_MIGRATIONS`, `AUTH_MIGRATIONS`, etc.). Generated entries use the filename stem as the migration ID, embed SQL with `include_str!`, and are sorted by the numeric prefix before the first `_`.
