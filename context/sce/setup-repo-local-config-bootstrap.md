# SCE setup repo-local config bootstrap

## Scope

Task `setup-repo-gate-and-local-config-bootstrap` T02 defines the repo-local `.sce/config.json` bootstrap behavior for `sce setup`.
...
- `cli/src/app.rs` calls `services::setup::bootstrap_repo_local_config(&current_dir)` in the `Command::Setup` dispatch, immediately after `ensure_git_repository`.

## Relationship to other setup contracts

- The git-repo gate (`ensure_git_repository`) was introduced in T01 of the same plan.
- The config bootstrap is independent of config install and hook install; it runs before both.
