...
`cliGeneratedInput` derivation invokes the repository's shared generated-input
producer from a declarative source containing the producer and canonical inputs.
The producer owns two-pass evaluation, nondeterminism rejection, and exact
SHA-256 inventories for both the payload and its canonical inputs. Native,
release, test, and Clippy derivations all receive the same output through
`SCE_CLI_GENERATED_INPUT_DIR`; Pkl is unavailable inside those Cargo derivations.
Host and musl dependency-only derivations and `cli-fmt` omit the handoff, so
generated-input changes invalidate generation and compiling outputs without
rebuilding dependency artifacts or formatting.
