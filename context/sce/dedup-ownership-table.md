...
| Plan role mission, hard boundaries, startup, and planning procedure | `agents["shared-context-plan"].canonicalBody` in `config/pkl/base/shared-content.pkl` | `config/.opencode/agent/Shared Context Plan.md`; composed into Claude `change-to-plan` command and Pi prompt | intentional/keep |
| Code role mission, hard boundaries, one-task execution flow, and feedback loop | `agents["shared-context-code"].canonicalBody` in `config/pkl/base/shared-content.pkl` | `config/.opencode/agent/Shared Context Code.md`; composed into Claude code-workflow commands and Pi prompts | intentional/keep |
