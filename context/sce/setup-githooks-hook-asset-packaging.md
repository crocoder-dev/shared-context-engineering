...
- the Rust `post-commit` runtime handles Agent Trace persistence after this handoff: DB insertion remains the required persistence path, successful git contexts write the validated full Agent Trace JSON best-effort to git notes under the configured Agent Trace notes ref (default `refs/notes/sce-agent-trace`), and successful local note writes attempt a silent fail-open push to the same `remote_url` unless `policies.agent_trace.push_notes.enabled` is `false`.
