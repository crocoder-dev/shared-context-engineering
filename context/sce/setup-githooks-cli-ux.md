...
- repository-required hook flows fail before config or hook writes when the target directory is not a Git repository, rendering `This directory is not a Git repository. Run \`git init\`, then rerun \`sce setup\`.` from the closed runtime user-error catalog; the technical repository-resolution source is retained for observability
