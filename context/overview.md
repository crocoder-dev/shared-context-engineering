...
Its command loop is implemented with `clap` derive-based argument parsing and `anyhow` error handling, with implemented auth flows (`auth login|logout|status`) plus auth-local guidance for bare `sce auth` / `sce auth --help`, implemented config inspection/validation (`config show`/`config validate`), real setup orchestration, implemented `doctor` rollout validation and location reporting, implemented `hooks` subcommand routing/validation entrypoints, implemented machine-readable runtime identification (`version`), implemented shell completion script generation via `clap_complete` (`completion --shell <bash|zsh|fish>`), and placeholder dispatch for deferred commands (`mcp`, `sync`) through explicit service contracts.
...
The config service now provides deterministic runtime config resolution with explicit precedence (`flags > env > config file > defaults`), strict config-file validation (`log_level`, `timeout_ms`, `workos_client_id`), deterministic default discovery/merge of global+local config files (`${global_config_root}/sce/config.json` then `.sce/config.json` with local override, where `global_config_root` comes from platform-appropriate `dirs` state/data directory resolution), a default `log_level` of `error`, shared auth-key resolution with optional baked defaults starting at `workos_client_id`, and deterministic text/JSON output contracts for `sce config show` and `sce config validate`.
...
- Use `context/sce/agent-trace-hook-doctor.md` for the implemented `sce doctor` contract across default/per-repo/global hook-path installs, including config-path and Agent Trace local DB location reporting.
