...
Its command loop is implemented with `lexopt` argument parsing and `anyhow` error handling, with implemented config inspection/validation (`config show`/`config validate`), real setup orchestration, implemented `doctor` rollout validation, implemented `hooks` subcommand routing/validation entrypoints, implemented machine-readable runtime identification (`version`), and placeholder dispatch for deferred commands (`mcp`, `sync`) through explicit service contracts.
