...
- **Exception exercised:** implementation revealed a sufficiently substantial shared mutation-scope health contract — the four status definitions, classifier ownership, doctor consumption semantics, severity/readiness/fixability mapping, and remediation/output shape — that a dedicated canonical document was clearer than embedding it into the already broad doctor operator contract. `context/sce/mutation-scope-health-status.md` therefore owns the shared health vocabulary and doctor-consumption contract; `context/sce/agent-trace-hook-doctor.md` links to it instead of duplicating it.
...
- [x] T06: `Report mutation-scope health in doctor JSON and human text output` (status:done)
  - Task ID: T06
  ...
  - Context synchronization: synced
  - Completed: 2026-09-19
  - Files changed: `cli/src/services/doctor/types.rs`, `cli/src/services/doctor/inspect.rs`, `cli/src/services/doctor/render.rs`, `cli/src/services/doctor/mod.rs`, `cli/src/services/doctor/fixes.rs`, `cli/src/services/lifecycle.rs`, `context/sce/mutation-scope-health-status.md`, `context/sce/agent-trace-hook-doctor.md`, `context/context-map.md`, `context/plans/doctor-mutation-scope-health.md`
  - Result: Wired the four adapters' T02–T05 `classify_health` functions into `sce doctor`. `types.rs` adds `MutationScopeHealthRow` (adapter target + shared `MutationScopeHealthStatus` + reason + detail) on `HookDoctorReport`, a new `ProblemCategory::MutationScopeHealth`, three new `ProblemKind` variants (`MutationScopeHealthRecovering`/`Blocked`/`Invalid`), a new `DoctorDisplayDetail::MutationScopeHealth` variant, `mutation_scope_health_status`/`mutation_scope_target_id` stringify helpers, a `compute_readiness` helper extracted from the previously inline readiness reduction, and a third `ProblemFixability::NoActionRequired` variant (stable JSON `"no_action_required"`) alongside `AutoFixable`/`ManualOnly`. `inspect.rs` adds `inspect_mutation_scope_health`, which resolves `git_dir` via `mutation_trace::runtime::resolve_git_dir` and, for every target `resolve_doctor_integration_targets` already resolves as configured/detected, calls that adapter's own `classify_health` (never writing to any state file) and pushes a `Warning`-severity `DoctorProblem` for `recovering` or an `Error`-severity one for `blocked`/`invalid` per AC6. `Blocked`/`Invalid` keep `ManualOnly` fixability and remediation text that names the real per-adapter state file path (via each adapter's own `state::state_path`), states plainly that no safe generic recovery command exists yet, and never recommends deleting the state file. `Recovering` now gets `ProblemFixability::NoActionRequired` (not `ManualOnly`) and a corrected, adapter-neutral generic remediation: it no longer claims the *next* mutation-capable tool call clears recovery (false for the Codex same-lane-successor and Pi duplicate-Start cases T03/T05 proved), and instead states only that a proven ordinary lifecycle/admission path exists, that not every subsequent call is necessarily recovery-capable for every adapter, that no manual action is required now, and to rerun `sce doctor` after further adapter activity if still recovering; the adapter classifier's own `reason` remains the authoritative specific explanation. `render.rs` renders a per-target "Mutation-scope health" row using the existing `[PASS]/[WARN]/[FAIL]` compact/expand convention and a top-level `mutation_scope_health` JSON array (`target`/`status`/`reason`/`detail`, using stable `claude`/`opencode`/`pi`/`codex` and `healthy`/`recovering`/`blocked`/`invalid` slugs). `mod.rs` now calls the shared `compute_readiness` helper instead of duplicating the reduction, and its `doctor_problem_fixability`/`health_problem_fixability` conversions are now exhaustive over three `ProblemFixability`/`HealthFixability` variants. `fixes.rs`'s `build_manual_fix_results` now filters to `fixability == ManualOnly` (was `!= AutoFixable`), so a `NoActionRequired` problem — currently only `Recovering` — produces no `--fix` result at all instead of a "Manual remediation is still required" one. `lifecycle.rs` gained a matching `HealthFixability::NoActionRequired` (no `ServiceLifecycle` provider currently diagnoses one; it exists only to keep `doctor/mod.rs`'s exhaustive conversions total) plus the pre-existing matching `HealthCategory`/`HealthProblemKind` variants, for the same reason as before: `doctor/mod.rs`'s conversions are exhaustive 1:1 matches even though mutation-scope problems are constructed directly in `inspect.rs`, never through a `ServiceLifecycle` provider.
  - Verify: `nix develop -c ./scripts/run-cli-cargo.sh test --manifest-path cli/Cargo.toml doctor` — pass (44 tests). Beyond the original per-field `mutation_scope_health_*` tests (now also asserting `fixability`/`next_action` per status) and the `render::tests` `[PASS]/[WARN]/[FAIL]`/JSON-slug coverage, this pass adds: `recovering_remediation_never_promises_a_specific_guaranteed_next_event` (asserts the generic remediation contains neither "next mutation-capable tool call" nor "the next tool call"); `mutation_scope_health_dispatches_all_four_configured_targets` (a repo with all four `.claude`/`.codex`/`.opencode`/`.pi` directories present and no state files dispatches the real per-adapter `classify_health` and yields exactly one `healthy` row per target, no problems); and four full-report regressions driving the actual `execute_doctor_with_context` → `render_report` path (text and JSON) against a real temporary git repository with a fully healthy canonical Claude integration (installed via `install_required_git_hooks` + `run_setup_for_mode`) plus a real persisted Claude adapter state file, isolating `XDG_STATE_HOME`/`XDG_CONFIG_HOME`/`XDG_CACHE_HOME` to a per-test sandbox (mutex-serialized) so the repository-scoped Agent Trace DB bootstrap this requires never touches the developer's real global SCE state: `full_report_recovering_regression_is_consistent_across_surfaces` (exactly one problem; `severity: warning`, `fixability: no_action_required`, `next_action: no_action_required`; human `[WARN] Mutation-scope health` and `Summary: 0 blocking problem(s), 1 warning(s)`; JSON `readiness: "ready"`, one `mutation_scope_health` row with `status: "recovering"`, one `problems` entry with `category: "mutation_scope_health"`/`severity: "warning"`/`fixability: "no_action_required"`/`remediation.next_action: "no_action_required"`), `full_report_recovering_fix_mode_produces_no_manual_repair_result` (running the same repository through `DoctorMode::Fix` produces no fix result in the `mutation_scope_health` category), `full_report_blocked_regression_is_consistent_across_surfaces` (readiness `not_ready`, `severity: error`, `fixability: manual_only`, `next_action: manual_steps`, human `[FAIL] Mutation-scope health` and `Summary: 1 blocking problem(s), 0 warning(s)`, remediation asserted to state `--fix` will not modify recovery state, that no safe generic recovery command exists, and to never recommend deleting the state file unqualified), and `full_report_invalid_matches_blocked_error_aggregation` (same error/not-ready/manual-only aggregation shape as `Blocked`, driven by a malformed state file). `nix develop -c ./scripts/run-cli-cargo.sh fmt --manifest-path cli/Cargo.toml --check` clean. `nix develop -c ./scripts/run-cli-cargo.sh clippy --manifest-path cli/Cargo.toml --all-targets -- -D warnings` clean. Because the shared `ProblemFixability`/`HealthFixability` enums gained a variant, `nix develop -c ./scripts/run-cli-cargo.sh test --manifest-path cli/Cargo.toml lifecycle` was also run — 69 passed, 0 failed (no other call site matches these enums exhaustively; all others are equality comparisons or struct-literal constructions, unaffected by the new variant). No T02–T05 adapter classification mapping, mutation-scope recovery/barrier behavior, or `doctor --fix` adapter-state mutation was changed.
  - Context impact: domain — implementation revealed a sufficiently substantial shared mutation-scope health contract (the four status definitions, classifier ownership, doctor consumption semantics, severity/readiness/fixability mapping, and remediation/output shape) that a dedicated canonical document, `context/sce/mutation-scope-health-status.md`, was clearer than embedding it into the already broad doctor operator contract — the explicit exception the plan's context-sync section already allowed, now exercised and recorded there rather than duplicated back into `context/sce/agent-trace-hook-doctor.md`. That document's "Doctor problem and readiness mapping" section now records the corrected three-way fixability split (`Blocked`/`Invalid` are `manual_only`; `Recovering` is the distinct `no_action_required`, never `manual_only`) and the corrected adapter-neutral `Recovering` remediation wording. `context/sce/agent-trace-hook-doctor.md` gained a `no_action_required` entry in its fixability-classes list and its mutation-scope-health bullet was corrected from claiming all statuses are "manual-only, non-`--fix`-able" to stating the actual three-way split. `context/context-map.md`'s existing entry for `mutation-scope-health-status.md` was corrected from "the manual-only never-delete-the-state-file remediation policy" (true only for `Blocked`/`Invalid`) to name the `no_action_required` fixability for `Recovering` too. No adapter-specific `context/cli/*-mutation-scope-integration.md` doc needed changes for this task (T02–T05 already recorded each adapter's proven mapping); this task only adds and corrects doctor's own consumption/reporting layer.
