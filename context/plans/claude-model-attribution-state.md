...
`direct > exact transcript > current session/agent state > NULL`. The register is a
...
- `context/sce/agent-trace-hooks-command-routing.md` — add `sce hooks claude-model-state`, its synchronous `SessionStart` and Claude-lifecycle-compatible asynchronous `PostModelSwitch` intake, the zero-stdout / fail-open / no-exit-2 / local-only contract, the post-switch visibility race and no-convergence-wait behavior, and the diff-trace `direct > transcript > state > NULL` precedence with ephemeral agent context and subagent isolation.
...
- [ ] T03: `Add silent sce hooks claude-model-state intake` (status:todo)
  - Task ID: T03
  - Scope: In — `claude-model-state` in `cli_schema.rs`, `command_runtime.rs`, `HookSubcommand`, and `run_hooks_subcommand`; raw `SessionStart`/`PostModelSwitch` parsing, validation, `cc_`/`claude/` normalization, optional ephemeral `agent_id` handling; injectable clock seam for local `observed_at_ms`; fail-open logging through the existing logger; zero stdout bytes, no exit 2, no denial, local-only/no-network/no-sync behavior on every branch; keep `SessionStart` synchronous and minimal/fast, and respect Claude's `PostModelSwitch` lifecycle semantics, including overlapping hook executions; a successful PostModelSwitch write makes state visible when its local DB write completes, not synchronously with Claude's model switch, and no causal ordering is asserted; focused tests for SessionStart (model / missing model / cleared-state protection), PostModelSwitch (to_model wins, `from_model` validation/diagnostics but no strict CAS, source variants, local-observation guard, replay idempotence, equal-time), and stdout-byte assertions. Out — diff-trace persistence lookup, generated settings, doctor/setup merge.
  - Dependencies: T02
  - Done when: both lifecycle events convert to guarded state writes with no stdout bytes, SessionStart remains synchronous and minimal/fast, PostModelSwitch respects Claude's lifecycle semantics, and a pre-`003` DB surfaces `Run 'sce setup'.` guidance without migrating.
...
- The state fallback remains a one-turn best-effort attribution and cannot detect a model substitution that Claude does not expose in the lifecycle event or transcript at lookup time.
