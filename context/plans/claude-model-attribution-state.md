...
- [x] T05: `Register and merge Claude lifecycle hooks safely` (status:done)
  ...
  - Result: Registered the Claude `SessionStart` and `PostModelSwitch` lifecycle events to the silent `sce hooks claude-model-state` command while preserving the existing five registrations; covered non-destructive/idempotent setup merging, stale SCE model-state replacement, and individual doctor drift repair for both lifecycle events.
  - Verify: `nix run .#pkl-check-generated` — pass (ephemeral generation: 141 files; inventory `93228c3eaa96c0cf4c9eb4f6c59d17a828bff73fb9be8235694dfe5e48b9efbf`); `nix develop -c ./scripts/run-cli-cargo.sh test --manifest-path cli/Cargo.toml config_merge` — pass (18 tests); `nix develop -c ./scripts/run-cli-cargo.sh test --manifest-path cli/Cargo.toml doctor` — pass (24 tests); `nix develop -c ./scripts/run-cli-cargo.sh test --manifest-path cli/Cargo.toml setup` — pass (66 tests). Compatibility smoke passed on Claude Code 2.1.251 and immediately older 2.1.250: both accepted a settings file containing unknown `PostModelSwitch`, returned the expected invalid-API-key termination rather than a settings/configuration failure, and left existing settings/hooks unchanged. Policy: keep unconditional `PostModelSwitch` registration; Claude Code 2.1.250 safely ignores the unknown event registration, so no raised minimum or capability gate is required.
  - Done checks: All satisfied — canonical Pkl output includes the two lifecycle registrations and existing hooks; setup merge preserves user keys/hooks, replaces stale SCE-owned model-state entries, and is idempotent; doctor reports missing `SessionStart` or `PostModelSwitch` as settings drift and repairs each through the existing merge path; compatibility smoke establishes unconditional installation compatibility for Claude Code 2.1.250 and newer.
  - Context impact: localized Claude setup/doctor integration contract and generated hook registration — context synchronization must document the two lifecycle registrations, compatibility policy, non-destructive merge/drift behavior, and inspect the mandatory root context files.
  - Context synchronization: synced
