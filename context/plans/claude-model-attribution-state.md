...
- [x] T03: `Add silent sce hooks claude-model-state intake` (status:done)
  ...
  - Verify: `nix develop -c ./scripts/run-cli-cargo.sh test --manifest-path cli/Cargo.toml claude_model_state` — pass (11 tests); `nix develop -c ./scripts/run-cli-cargo.sh test --manifest-path cli/Cargo.toml claude_model` — pass (15 tests, including existing Claude attribution coverage); direct malformed and model-less hook smoke checks — pass (exit 0, stdout 0 bytes).
  - Done checks: All satisfied — CLI/runtime routing recognizes `claude-model-state`; lifecycle payload parsing handles normalized SessionStart/PostModelSwitch state, optional agent scopes, all declared switch sources, to-model precedence, and no-op/malformed cases; direct guarded writes use the no-migration hook DB path; failures remain logger-only, empty-stdout, fail-open behavior.
