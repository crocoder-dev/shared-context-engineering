...
- [x] T11: `Normalize workflow language and document format` (status:done)
  - Scope: shared workflow vocabulary and its domain authority, embedded-phase/reference wording, output/report terminology, plan-authoring tone, handover persisted-document format, validation non-repairing wording, canonical Pkl, generated Pi/Claude/Codex mirrors, semantic generation checks, and ownership context.
  - Vocabulary after change: `context/sce/workflow-vocabulary.md` is the single domain authority for command/workflow/skill/phase/step, internal-result/report/handoff terminology, lifecycle words, verbs, and document roles.
