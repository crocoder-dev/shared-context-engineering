...
- [x] T02: `Repoint the canonical config schema URL at the config subdomain` (status:done)
  - Task ID: T02
  ...
  - Completed: 2026-08-07
  - Files changed: `config/pkl/base/sce-config-schema.pkl`, `cli/src/services/agent_trace.rs`, `cli/src/services/setup/mod.rs`, `.sce/config.json`
  - Evidence: the generated schema declares `$id` `https://config.sce.crocoder.dev/config.json` and a two-value `$schema` `enum` of the new and old URLs; `nix run .#pkl-check-generated` passed (101 files, inventory sha256 `cdb716a0…`); `nix flake check` passed all six checks; a `nix build .#sce` binary reported valid for configs declaring each URL and rejected `https://example.com/config.json` with `is not one of` both accepted values; `sce setup --claude --non-interactive` in a scratch git repo wrote `{"$schema": "https://config.sce.crocoder.dev/config.json"}`, which `sce config validate` accepted.
  - Notes: `README.md`, `AGENTS.md`, and `cli/README.md` needed no change — every `sce.crocoder.dev` occurrence in them is a docs-site link, not the config schema URL. Deviation: the Rust schema tests covering both accepted `$schema` values were written and then removed at the user's explicit request during implementation, so AC4's clause "the corresponding Rust schema test covers both accepted values" is currently unmet; the behavior itself is proven by the `sce config validate` runs above.
