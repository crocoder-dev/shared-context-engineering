...
<<<<<<< HEAD
- `cli/src/command_surface.rs` is the source of truth for top-level command contract metadata (`help`, `config`, `setup`, `doctor`, `auth`, `hooks`, `trace`, `sync`, `version`, `completion`), including implementation status for command classification and per-command visibility on the slim top-level help surface.
