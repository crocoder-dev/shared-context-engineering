...
- `cli/src/app.rs` provides a `lexopt`-based argument parser and dispatch loop with deterministic help/setup execution plus stable class-based exit-code mapping (`2` parse, `3` validation, `4` runtime, `5` dependency).
- `cli/src/services/observability.rs` provides deterministic runtime observability controls and rendering for app lifecycle logs, including env-configured threshold/format (`SCE_LOG_LEVEL`, `SCE_LOG_FORMAT`), stable event identifiers, severity filtering, and stderr-only emission.
