...
- `cli/src/services/agent_trace.rs` defines the Agent Trace schema adapter contract (`adapt_trace_payload`) that maps internal attribution structures into Agent Trace-shaped records with fixed git VCS identity and reserved reverse-domain metadata keys.
