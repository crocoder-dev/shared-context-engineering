...
- `cli/src/app.rs` provides the clap-based argument dispatch loop with deterministic help/setup execution, bare-command help routing for `sce auth` and `sce config`, centralized stream routing (`stdout` success payloads, `stderr` redacted diagnostics), startup observability bootstrapping that now tolerates invalid default-discovered config files by continuing with degraded defaults plus `sce.config.invalid_config` warn-level logs, stable class-based exit-code mapping (`2` parse, `3` validation, `4` runtime, `5` dependency), and stable class-based stderr diagnostic codes (`SCE-ERR-PARSE`, `SCE-ERR-VALIDATION`, `SCE-ERR-RUNTIME`, `SCE-ERR-DEPENDENCY`) with default `Try:` remediation injection when missing.
...
- `cli/src/services/local_db.rs` provides the local SQLite persistence service for Agent Trace milestone 1, including repo-local `.sce/local.db` initialization, idempotent schema bootstrap for `sessions`/`conversations`/`prompts`/`assistant_messages`/`file_observations`/`file_ranges`/`trace_exports`, UUID + RFC3339 UTC record shaping, typed CRUD/query helper APIs, and submit-time orchestration helpers for lazy init + active session/conversation reuse (`ensure_db_initialized`, `ensure_active_session`, `ensure_active_conversation`, `append_prompt_with_active_context`, `append_prompt_with_auto_init`).
...
- `cli/src/services/trace.rs` defines the prompt-persistence bridge command (`sce trace append-prompt`) and routes prompt text to `local_db::append_prompt_with_auto_init(...)` from the current repository root, preserving lazy DB initialization plus active session/conversation ensure in the Rust persistence layer.
...
- `cli/src/services/` contains module boundaries for config, setup, doctor, hooks, trace, sync, version, completion, and local DB adapters with explicit trait seams for future implementations.
...
This phase establishes compile-safe extension seams with a dependency baseline (`anyhow`, `chrono`, `clap`, `clap_complete`, `dirs`, `hmac`, `inquire`, `jsonschema`, `opentelemetry`, `opentelemetry-otlp`, `opentelemetry_sdk`, `reqwest`, `rusqlite`, `serde`, `serde_json`, `sha2`, `tokio`, `tracing`, `tracing-opentelemetry`, `tracing-subscriber`, `uuid`); local SQLite persistence foundations and submit-time orchestration helpers exist in `cli/src/services/local_db.rs`, and submit-path wiring is now active through `cli/src/services/trace.rs` plus the OpenCode plugin `chat.message` bridge.
