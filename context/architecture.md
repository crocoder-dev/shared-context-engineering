...
- `cli/src/command_surface.rs` is the source of truth for top-level command contract metadata (`help`, `config`, `setup`, `doctor`, `mcp`, `hooks`, `sync`, `version`, `completion`) and explicit implemented-vs-placeholder status.
