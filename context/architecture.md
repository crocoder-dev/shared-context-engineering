...
- `cli/src/services/local_db.rs` provides the local Turso data adapter, including `Builder::new_local(...)` initialization, async execute/query smoke checks for in-memory and file-backed targets, and idempotent core schema migration application for Agent Trace persistence foundations (`repositories`, `commits`, `trace_records`, `trace_ranges`).
