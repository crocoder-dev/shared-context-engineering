...
resolved by `runtime::git_snapshot::resolve_git_dir`. Two linked worktrees resolve to two
...
Durability follows the repository's established durable-file pattern: it protects against
...
resolve git_dir → acquire WorktreeLock
  ...
  → resolve_worktree_id from Git topology
...
worktree-identity failure, DB read/write failure, CAS exhaustion, scope-identity
...
- `ExternalTaintMarker { operation: ExternalTaintOperation, source }` where
  ...
  `Persist` failure is returned **before** any worktree-identity, DB, snapshot,
