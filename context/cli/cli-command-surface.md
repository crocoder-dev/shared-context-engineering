...
`hooks` routes through implemented subcommand parsing/dispatch for `pre-commit`, `commit-msg`, `post-commit`, `post-rewrite`, and `diff-trace`; current behavior remains disabled by default for commit attribution, while `diff-trace` is active STDIN intake with required non-empty `sessionID`/`diff` plus required `u64` `time` validation, collision-safe artifact writes, and AgentTraceDb insertion, and `post-commit` persists latest-session `ParsedPatch` intersections into `patch_intersections` when usable diff traces and a valid `HEAD` patch exist.
...
- `cli/src/services/hooks/mod.rs` defines production local hook runtime parsing/dispatch (`HookSubcommand`, `run_hooks_subcommand`) for `pre-commit`, `commit-msg`, `post-commit`, `post-rewrite`, and `diff-trace`; `cli/src/services/hooks/command.rs` owns the hook runtime command handler. Current runtime behavior is commit-msg-only attribution behind the disabled-default attribution gate, `pre-commit`/`post-rewrite` deterministic no-ops, `diff-trace` STDIN JSON intake with collision-safe artifact plus AgentTraceDb raw-diff insertion, and `post-commit` latest-session `ParsedPatch` intersection persistence into `patch_intersections` with deterministic no-data and runtime-error diagnostics. `cli/src/services/hooks/lifecycle.rs` implements `ServiceLifecycle` for hook health checks, fix, and setup (hook rollout integrity and required-hook installation).
...
- `cli/src/services/agent_trace_db/mod.rs` provides `AgentTraceDb = TursoDb<AgentTraceDbSpec>` plus `DiffTraceInsert<'_>`/`insert_diff_trace()` for parameterized writes to `diff_traces` and `PatchIntersectionInsert<'_>`/`insert_patch_intersection()` for parameterized writes to `patch_intersections`.
- `AgentTraceDb::new()` resolves `<state_root>/sce/agent-trace.db` through `default_paths::agent_trace_db_path()`, creates parent directories through `TursoDb`, opens the Turso database, and runs embedded ordered Agent Trace DB migrations for `diff_traces` and `patch_intersections`.
