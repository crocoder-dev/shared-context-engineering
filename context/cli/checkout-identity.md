...
- `cli/src/services/checkout/mod.rs`
  ...
  - `resolve_or_create_agent_trace_db_for_checkout(repo_root)` gets or creates checkout identity, resolves `<state_root>/sce/project-{checkout_id}.db`, fast-opens an existing ready DB, and falls back to migration-running initialization when the DB is absent or schema metadata is incomplete.
...
- `default_paths::agent_trace_db_path_for_checkout(checkout_id)` computes `<state_root>/sce/project-{checkout_id}.db`.
...
The global `project.db` path remains only as a lifecycle fallback when no checkout context or checkout ID is available. `sce doctor` displays the current checkout ID and per-checkout Agent Trace DB status when a checkout ID exists, and `sce doctor dbs` discovers project DBs by scanning `<state_root>/sce/project-*.db` files on disk, sorted by mtime descending.
