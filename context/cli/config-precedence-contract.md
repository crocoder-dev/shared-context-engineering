...
- `show` includes resolved Agent Trace hook policy under `result.resolved.policies.agent_trace`, including `git_notes_ref` and `push_notes.enabled` with source metadata.
