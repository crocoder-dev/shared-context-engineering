...
- `context/sce/local-db.md` (implemented `cli/src/services/local_db.rs` Turso adapter with `LocalDb` struct, embedded migrations via `include_str!` executed through Turso batch execution, deterministic retired-placeholder schema reset/fail-fast bootstrap handling, blocking `execute`/`query` methods using a tokio current-thread runtime, typed `insert_agent_trace(&AgentTrace)` full-payload plus normalized-row persistence with rollback-on-failure and duplicate-ID primary-key failure behavior, and the current normalized Agent Trace persistence schema in `001_create_agent_traces.sql`)
- `context/sce/agent-trace-core-schema-migrations.md` (historical reference for the retired empty-file/no-schema local DB baseline; current active schema notes live in `context/sce/local-db.md`)
