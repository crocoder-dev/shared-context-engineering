...
- `context/sce/agent-trace-db.md` (implemented `cli/src/services/agent_trace_db/mod.rs` Agent Trace database wrapper with canonical `<state_root>/sce/agent-trace.db` path, ordered `diff_traces`, nullable `diff_traces.model_id`, `post_commit_patch_intersections`, and `diff_traces(time_ms, id)` index migrations applied through shared migration metadata, typed parameterized insert helpers for diff traces and post-commit intersection rows, inclusive bounded chronological recent `diff_traces` query/parse support with nullable `model_id` projection and malformed-row skip accounting, registered setup/doctor lifecycle provider, and active `sce hooks diff-trace` dual-write persistence)
