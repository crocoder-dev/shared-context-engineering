...
- `context/cli/service-lifecycle.md` (implemented lifecycle capability foundation in `cli/src/services/lifecycle.rs`, hooks and local DB lifecycle registry in `cli/src/services/lifecycle_registry.rs`, hooks lifecycle service in `cli/src/services/hooks_lifecycle.rs` for required git-hook setup, diagnosis, fix, and internal preview/action-plan data, and local DB lifecycle service in `cli/src/services/local_db_lifecycle.rs` for local DB path resolution, parent-directory readiness, and DB health diagnosis; setup hooks, doctor hook diagnosis, and doctor hook repair all resolve/consume hooks lifecycle capabilities through the registry; doctor local DB diagnosis resolves/consumes local DB diagnostic lifecycle capabilities through the registry; doctor local DB repair resolves/consumes local DB fix lifecycle capabilities through the registry; local DB diagnostic and fix capabilities are registered in the registry; extension pattern for future services is documented)
