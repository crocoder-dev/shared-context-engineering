...
- `context/sce/local-db.md` (implemented `cli/src/services/local_db.rs` Turso adapter with `LocalDb` struct, embedded `diff_traces` migration via `include_str!`, blocking `execute`/`query` methods, typed `DiffTraceInsert`/`LocalDb::insert_diff_trace` parameterized insertion seam using a tokio current-thread runtime, and current `sce hooks diff-trace` consumer behavior)
