...
- `context/cli/service-lifecycle.md` (implemented lifecycle capability foundation in `cli/src/services/lifecycle.rs`, hook-only lifecycle registry in `cli/src/services/lifecycle_registry.rs`, and hooks lifecycle service in `cli/src/services/hooks_lifecycle.rs` for required git-hook setup, diagnosis, fix, and internal preview/action-plan data; setup hooks, doctor hook diagnosis, and doctor hook repair all resolve/consume hooks lifecycle capabilities through the registry; extension pattern for future services is documented)
