...
- `context/sce/agent-trace-post-commit-dual-write.md` (current post-commit intersection-only boundary plus historical dual-write/full-Agent-Trace reference)
...
- `context/sce/agent-trace-db.md` (implemented `cli/src/services/agent_trace_db/mod.rs` Agent Trace database wrapper with canonical `<state_root>/sce/agent-trace.db` path, embedded `diff_traces` and `patch_intersections` migrations, `DiffTraceInsert<'a>`/`DiffTracePatchRow`/`PatchIntersectionInsert<'a>`, parameterized insert helpers, deterministic latest-session/session-patch read helpers, registered setup/doctor lifecycle provider, active `sce hooks diff-trace` raw diff persistence, and active `sce hooks post-commit` patch-intersection persistence)
...
- `context/sce/agent-trace-minimal-generator.md` (implemented library-only Agent Trace seam at `cli/src/services/agent_trace.rs`, including minimal Agent Trace JSON generation with `version`, UUIDv7 `id`, caller-provided commit-time `timestamp`, and `post_commit_patch`-anchored hunk classification, plus pure `build_patch_intersection_json(...)` support that turns ordered stored diff-trace patches and a post-commit patch into serialized `ParsedPatch` intersection JSON with ordered source IDs for later DB insertion)
- `context/sce/agent-trace-hooks-command-routing.md` (implemented `sce hooks` command routing plus current runtime behavior: disabled-default commit-msg attribution, no-op `pre-commit`/`post-rewrite`, `diff-trace` STDIN intake with required-field validation plus dual persistence to AgentTraceDb and collision-safe `context/tmp/<timestamp>-000000-diff-trace.json` artifacts, and `post-commit` latest-session `ParsedPatch` intersection persistence)
