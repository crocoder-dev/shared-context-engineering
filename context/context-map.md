...
- `context/sce/setup-repo-local-config-bootstrap.md` (setup local bootstrap behavior: repo-local `.sce/config.json` create-if-missing plus setup-owned `local.db` and `agent-trace.db` initialization before config/hooks dispatch across setup modes)
...
- `context/sce/local-db.md` (implemented tableless `cli/src/services/local_db.rs` Turso adapter with `LocalDb` struct, empty migration list, blocking `execute`/`query` methods, and tokio current-thread runtime; setup/doctor still use `LocalDb::new()`, while active hook diff-trace persistence uses `AgentTraceDb`)
- `context/sce/agent-trace-db.md` (implemented `cli/src/services/agent_trace_db.rs` Turso adapter with `AgentTraceDb` struct, embedded `diff_traces` migration from `cli/migrations/agent-trace/`, blocking `execute`/`query` methods, typed `DiffTraceInsert`/`AgentTraceDb::insert_diff_trace` parameterized insertion seam using a tokio current-thread runtime, active `sce hooks diff-trace` persistence through `agent-trace.db`, and setup-time `bootstrap_agent_trace_db()` initialization from app setup dispatch)
...
- `context/sce/agent-trace-hooks-command-routing.md` (implemented `sce hooks` command routing plus current runtime behavior: disabled-default commit-msg attribution, no-op `pre-commit`/`post-commit`/`post-rewrite` entrypoints, and `diff-trace` STDIN intake with required-field validation, collision-safe `context/tmp/<timestamp>-000000-diff-trace.json` persistence, plus accepted-payload insertion into `agent-trace.db` `diff_traces`)
...
- `context/sce/opencode-agent-trace-plugin-runtime.md` (current OpenCode agent-trace plugin runtime behavior, including `session.diff`-only capture, guarded `{ sessionID, diff, time }` extraction, and CLI handoff to `sce hooks diff-trace` over STDIN JSON so Rust hook runtime owns collision-safe diff-trace artifact writes plus additive `agent-trace.db` insertion)
