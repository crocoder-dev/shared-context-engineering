...
- [x] T01: `{title}` (status:done)
  ...
  - Context synchronization handoff: Changed files: {paths}; Implementation summary: {summary}; Verification: {commands and outcomes}; Done checks: {status of each done check}; Context impact: {durable context this change affects, or none}
