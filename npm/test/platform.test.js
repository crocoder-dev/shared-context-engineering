...
describe("resolveSupportedPlatform", () => {
  ...
  test("returns null for unsupported platforms", () => {
    ...
    expect(formatUnsupportedPlatformMessage("linux", "arm64")).toContain("linux/arm64");
  });
});

describe("release naming helpers", () => {
  test("derive archive names from version and target triple", () => {
    expect(getArchiveRoot("0.1.0", "x86_64-unknown-linux-gnu")).toBe("sce-v0.1.0-x86_64-unknown-linux-gnu");
    expect(getArchiveName("0.1.0", "x86_64-unknown-linux-gnu")).toBe(
      "sce-v0.1.0-x86_64-unknown-linux-gnu.tar.gz",
    );
    expect(getReleaseManifestName("0.1.0")).toBe("sce-v0.1.0-release-manifest.json");
  });
});
...
describe("selectReleaseArtifact", () => {
  ...
  test("throws when target triple is absent", () => {
    expect(() =>
      selectReleaseArtifact(
        { artifacts: [{ target_triple: "x86_64-apple-darwin", archive: "macos.tgz" }] },
        "x86_64-unknown-linux-gnu",
      ),
    ).toThrow("No sce release artifact found for target x86_64-unknown-linux-gnu.");
  });
});
