...
function createManifestFixture(overrides = {}) {
  const manifest = {
    version: "0.1.0",
    artifacts: [{ target_triple: "x86_64-unknown-linux-gnu", checksum_sha256: "abc123" }],
    ...overrides,
  };
  ...
  const signature = sign("sha256", Buffer.from(manifestPayload), privateKey).toString("base64");
...
describe("release manifest verification primitives", () => {
  ...
  test("accepts a valid detached manifest signature", () => {
    ...
    expect(verifyReleaseManifestSignature(manifestPayload, signature, publicKey)).toBe(true);
  });
  ...
  test("rejects a tampered manifest payload", () => {
    ...
    expect(verifyReleaseManifestSignature(tamperedPayload, signature, publicKey)).toBe(false);
  });
  ...
  test("rejects a tampered signature payload", () => {
    ...
    const manifestPayload = JSON.stringify({ version: "0.1.0", artifacts: [] });
    const signatureBuffer = sign("sha256", Buffer.from(manifestPayload), privateKey);
    signatureBuffer[0] ^= 0xff;

    expect(
      verifyReleaseManifestSignature(manifestPayload, signatureBuffer.toString("base64"), publicKey),
    ).toBe(false);
  });
  ...
  test("accepts signatures produced by the release signing helper", () => {
    ...
    const signature = createReleaseManifestSignature(manifestPayload, privateKey);
  ...
  test("loads the release signing key from the configured environment variable", () => {
    ...
    expect(resolveReleaseManifestSigningKey({ env: { [SIGNING_KEY_ENV_VAR]: privateKeyPem } })).toBe(privateKeyPem);
  });
});

describe("verified release manifest loading", () => {
  test("parses and returns a valid signed manifest payload", async () => {
    const { manifest, manifestPayload, publicKey, signature } = createManifestFixture();
  ...
  test("rejects an invalid signed manifest payload", () => {
    ...
    expect(() => parseVerifiedReleaseManifest(manifestPayload, "not-a-valid-signature", publicKey)).toThrow(
    ...
    await expect(
      loadVerifiedReleaseManifest("0.1.0", {
        ...
        downloadManifestSignature: async () => {
          throw new Error("Unexpected response 404 while downloading signature.");
        },
        publicKeyPem: publicKey,
      }),
    ).rejects.toThrow(
...
describe("npm installer trust flow", () => {
  test("aborts before archive download when manifest signature verification fails", async () => {
    ...
    await expect(
      installBinaryWithDependencies({
        supportedPlatform: { targetTriple: "x86_64-unknown-linux-gnu", os: "linux", arch: "x64" },
        version: "0.1.0",
        loadReleaseManifest: () =>
          loadVerifiedReleaseManifest("0.1.0", {
            downloadManifest: async () => manifestPayload,
    ...
    ).rejects.toThrow("Release manifest authenticity check failed: signature verification did not succeed.");
  ...
  test("fails archive checksum verification only after manifest verification succeeds", async () => {
    ...
    const expectedChecksum = createHash("sha256").update(archiveContents).digest("hex");
    const { manifestPayload, publicKey, signature } = createManifestFixture({
      artifacts: [{ target_triple: "x86_64-unknown-linux-gnu", checksum_sha256: expectedChecksum }],
    });
    let extractCalls = 0;
  ...
  test("installs after verifying the manifest signature and archive checksum", async () => {
    ...
    const archiveName = `sce-v${version}-${targetTriple}.tar.gz`;
    ...
    const tempDir = path.join(tmpdir(), `sce-npm-install-test-${Date.now()}-${Math.random()}`);
    ...
    await expect(
      installBinaryWithDependencies({
        ...
        extractArchiveFn: (_archivePath, destinationDir) => {
          const extractedBinaryPath = path.join(destinationDir, `sce-v${version}-${targetTriple}`, "bin", "sce");
    ...
    expect(installedBinaryPaths[0].sourcePath).toBe(path.join(tempDir, `sce-v${version}-${targetTriple}`, "bin", "sce"));
    expect(installedBinaryPaths[0].destinationPath).toBe(path.join("/home/davidabram/repos/shared-context-engineering/master/npm", "runtime", "sce"));
  });
});
