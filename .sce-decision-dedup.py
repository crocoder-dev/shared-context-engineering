...
tracked_rels = [
    "skills/sce-decision/SKILL.md",
    "skills/sce-decision/references/adr-template.md",
    "skills/sce-next-task/references/context-sync.md",
]
before = {
    target: sum(len((Path(target) / rel).read_text().splitlines()) for rel in tracked_rels)
...
text = once(
    text,
    'local SKILL_DESCRIPTION = "Write one immutable ADR for one qualifying system-wide decision"',
    'local SKILL_DESCRIPTION = "Write one immutable ADR for one decision already qualified by context synchronization"',
...
local forbiddenDecisionTemplateGateTokens = new Listing {
  "System boundaries or ownership"
  "Public or cross-domain interfaces"
  "Data models or persistence"
  "Compatibility contracts"
      ...
      let (skillPath = "config/\(target)/skills/sce-decision/SKILL.md")
        let (templatePath = "config/\(target)/skills/sce-decision/references/adr-template.md")
          let (syncPath = "config/\(target)/skills/sce-next-task/references/context-sync.md")
            ...
            && requiredDecisionSkillTokens.every((token) -> documents[skillPath].contains(token))
...
for target in TARGETS:
    ...
    if not all(token in sync for token in criteria):
        raise SystemExit(f"{target}: decision gate lost qualification criteria")
    if "sole owner of decision qualification" not in sync:
        raise SystemExit(f"{target}: decision gate ownership marker missing")
    if any(token in skill for token in criteria) or any(token in skill for token in old_section_rules):
        raise SystemExit(f"{target}: decision skill still restates owned policy")
    for token in [
        "## Qualification handoff",
        "solely owns the decision\nqualification threshold",
        "sole authority for\nthe persisted ADR schema and section semantics",
        ...
        if token not in skill:
            raise SystemExit(f"{target}: decision skill missing consumer token {token!r}")
    for token in [
        "Status: {validated decision status}",
        "## Context",
        "## Decision",
        ...
        if token not in template:
            raise SystemExit(f"{target}: ADR template missing {token!r}")
    if "Proposed|Accepted|Rejected|Deprecated|Superseded" in template:
        raise SystemExit(f"{target}: template still owns status enum")
...
planned = {
    "config/pkl/base/decision-skill.pkl",
...
for target in TARGETS:
    planned |= {
        f"{target}/skills/sce-decision/SKILL.md",
        f"{target}/skills/sce-decision/references/adr-template.md",
